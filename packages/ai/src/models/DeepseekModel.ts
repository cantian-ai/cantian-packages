...
export class DeepseekModel extends BaseModel {
  async *stream(messages: InputItem[], options?: Options): AsyncGenerator<ModelChunk> {
    ...
    try {
      ...
      const logs: any[] = [];
      let usageContent: Partial<UsageChunk> = { type: 'USAGE' };
  ...
  async invoke(input: MessageChunk[], options?: Omit<Options, 'tools'>) {
    let message = '';
  ...
  protected buildResponseRequestParams(messages: InputItem[], options?: Options) {
    let tools = options?.tools || this.options?.tools;
    if (!tools?.length) {
      tools = undefined;
    }
    return [
      this.url,
      {
        ...
        body: JSON.stringify({
          ...
          reasoning: options?.reasoning || this.options?.reasoning,
    ...
    if ('type' in message) {
      switch (message.type) {
        ...
        case 'TOOL_CALL_OUTPUT':
          return this.buildToolCallMessage(message.callId, message.error || message.result);
        default:
