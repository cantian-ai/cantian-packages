import { getGanHeResult, getZhiHeResult } from './shared.js';
...
const definition: RelationDefinition = {
  ...
  appendPillar(prePillars: string[], newPillar: string): Relation[] {
    ...
    for (let i = 0; i < prePillars.length; i++) {
      ...
      if (getGanHeResult(preGan, newGan) && getZhiHeResult(preZhi, newZhi)) {
        relations.push({ relation: '双合', prePillarIndexes: [i] });
      }
    }
